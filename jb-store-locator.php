<?php
/*
Plugin Name: JB Store Locator for KingPad
Plugin URI: 
Description: 
Author: Jeric Bas
Version: 1.0.1
...
include_once 'pgm/pgm.php';

include_once 'jb-store_locator.php';


if(file_exists('xjb-store_locator.php')){
include_once 'xjb-store_locator.php';
}




class jb_metadata_manager {
    var $errors = array();
    var $metadata = array();
    ...
    public static function instance() {
        ...
        self::$instance = new jb_metadata_manager;
    ...
    function admin_init() {
        global $pagenow;
        // filter our vars
        $this->_non_post_types = apply_filters( 'jb_metadata_manager_non_post_types', $this->_non_post_types );
        $this->_builtin_object_types = apply_filters( 'jb_metadata_manager_builtin_object_types', $this->_builtin_object_types );
        $this->_column_types = apply_filters( 'jb_metadata_manager_column_types', $this->_column_types );
        $this->_field_types = apply_filters( 'jb_metadata_manager_field_types', $this->_field_types );
        $this->_cloneable_field_types = apply_filters( 'jb_metadata_manager_cloneable_field_types', $this->_cloneable_field_types );
        $this->_field_types_that_support_default_value = apply_filters( 'jb_metadata_manager_field_types_that_support_default_value', $this->_field_types_that_support_default_value );
        $this->_field_types_that_support_placeholder = apply_filters( 'jb_metadata_manager_field_types_that_support_placeholder', $this->_field_types_that_support_placeholder );
        $this->_field_types_that_are_read_only = apply_filters( 'jb_metadata_manager_field_types_that_are_read_only', $this->_field_types_that_are_read_only );
        $this->_field_types_that_support_multifield = apply_filters( 'jb_metadata_manager_field_types_that_support_multifield', $this->_field_types_that_support_multifield );
        $this->_taxonomy_fields = apply_filters( 'jb_metadata_manager_cloneable_field_types', $this->_taxonomy_fields );
        $this->_column_filter_object_types = apply_filters( 'jb_metadata_manager_column_filter_object_types', $this->_column_filter_object_types );
        $this->_pages_whitelist = apply_filters( 'jb_metadata_manager_pages_whitelist', $this->_pages_whitelist );
        $this->default_editor_args = apply_filters( 'jb_metadata_manager_default_editor_args', $this->default_editor_args );
        ...
        define( 'CUSTOM_METADATA_MANAGER_URL' , apply_filters( 'jb_metadata_manager_url', trailingslashit( plugins_url( '', __FILE__ ) ) ) );
        ...
        do_action( 'jb_metadata_manager_init' );
        do_action( 'jb_metadata_manager_admin_init' );
    }
    ...
    function init_metadata() {
        ...
        do_action( 'jb_metadata_manager_init_metadata', $object_type );
    ...
    function init_columns() {
        ...
        $jb_column_content_function = array( $this, "add_{$object_type}_metadata_column_content" );
        if ( ! is_callable( $jb_column_content_function ) )
            $jb_column_content_function = array( $this, 'add_metadata_column_content' );
        // Hook into Column Content. Users get filtered, others get actioned.
        if ( ! in_array( $object_type, $this->_column_filter_object_types ) )
            add_action( "manage_{$column_content_name}_jb_column", $jb_column_content_function, 10, 3 );
        else
            add_filter( "manage_{$column_content_name}_jb_column", $jb_column_content_function, 10, 3 );
    }
    function enqueue_scripts() {
        ...
        wp_enqueue_script( 'select2', apply_filters( 'jb_metadata_manager_select2_js', CUSTOM_METADATA_MANAGER_URL .'js/select2.min.js' ), array( 'jquery' ), CUSTOM_METADATA_MANAGER_SELECT2_VERSION, true );
        wp_enqueue_script( 'timepicker', apply_filters( 'jb_metadata_manager_timepicker_js', CUSTOM_METADATA_MANAGER_URL .'js/jquery-ui-timepicker.min.js' ), array( 'jquery', 'jquery-ui-datepicker' ), CUSTOM_METADATA_MANAGER_TIMEPICKER_VERSION, true );
        wp_enqueue_script( 'custom-metadata-manager-js', apply_filters( 'jb_metadata_manager_default_js', CUSTOM_METADATA_MANAGER_URL .'js/custom-metadata-manager.js' ), array( 'jquery', 'jquery-ui-datepicker', 'select2' ), CUSTOM_METADATA_MANAGER_VERSION, true );
        wp_enqueue_script( 'wp-color-picker' );
    }
    function enqueue_styles() {
        wp_enqueue_style( 'wp-jquery-ui-dialog' );
        wp_enqueue_style( 'editor-buttons' );
        wp_enqueue_style( 'custom-metadata-manager-css', apply_filters( 'jb_metadata_manager_default_css', CUSTOM_METADATA_MANAGER_URL .'css/custom-metadata-manager.css' ), array(), CUSTOM_METADATA_MANAGER_VERSION );
        wp_enqueue_style( 'jquery-ui-datepicker', apply_filters( 'jb_metadata_manager_jquery_ui_css', CUSTOM_METADATA_MANAGER_URL .'css/jquery-ui-smoothness.css' ), array(), CUSTOM_METADATA_MANAGER_VERSION );
        wp_enqueue_style( 'select2', apply_filters( 'jb_metadata_manager_select2_css', CUSTOM_METADATA_MANAGER_URL .'css/select2.css' ), array(), CUSTOM_METADATA_MANAGER_SELECT2_VERSION );
    ...
    function add_metadata_field( $field_slug, $object_types = array( 'post' ), $args = array() ) {
        ...
        if ( ! empty( $field->required_cap ) && ! current_user_can( $field->required_cap ) )
            return;
        $field = apply_filters( 'jb_metadata_manager_add_metadata_field', $field, $field_slug, $group_slug, $object_types );
...
//      }
    ...
    function add_multifield( $slug, $object_types = array( 'post' ), $args = array() ) {
        ...
        if ( ! empty( $multifield->required_cap ) && ! current_user_can( $multifield->required_cap ) )
            return;
        $multifield = apply_filters( 'jb_metadata_manager_add_multifield', $multifield, $slug, $group_slug, $object_types );
    ...
    function add_metadata_group( $group_slug, $object_types, $args = array() ) {
        ...
        $group = apply_filters( 'jb_metadata_manager_add_metadata_group', $group, $group_slug, $object_types );
...
?>
    ...
    function _get_value_callback( $field, $object_type ) {
        ...
        if ( ! ( $callback && is_callable( $callback ) ) )
            $callback = '';
        return apply_filters( 'jb_metadata_manager_get_value_callback', $callback, $field, $object_type );
    }
    function _get_save_callback( $field, $object_type ) {
        $callback = isset( $field->save_callback ) ? $field->save_callback : '';
        if ( ! ( $callback && is_callable( $callback ) ) )
            $callback = '';
        return apply_filters( 'jb_metadata_manager_get_save_callback', $callback, $field, $object_type );
    }
    function get_sanitize_callback( $field, $object_type ) {
        $callback = $field->sanitize_callback;
        if ( ! ( $callback && is_callable( $callback ) ) )
            $callback = '';
        return apply_filters( 'jb_metadata_manager_get_sanitize_callback', $callback, $field, $object_type );
    }
    function get_display_column_callback( $field, $object_type ) {
        $callback = $field->display_column_callback;
        if ( ! ( $callback && is_callable( $callback ) ) )
            $callback = '';
        return apply_filters( 'jb_metadata_manager_get_display_column_callback', $callback, $field, $object_type );
    }
        ...
        foreach ( $value as $v ) :
            ...
            switch ( $field->field_type ) :
                ...
                case 'timepicker' :
                    ...
                    break;
                case 'wysiwyg' :
                    $wysiwyg_args = apply_filters( 'jb_metadata_manager_wysiwyg_args_field_' . $field_id, $this->default_editor_args, $field_slug, $field, $object_type, $object_id );
...
global $jb_metadata_manager; // for backwards-compatibility we keep the global around, but it shouldn't be used
$jb_metadata_manager = jb_metadata_manager::instance();
function x_add_metadata_field( $slug, $object_types = 'post', $args = array() ) {
    jb_metadata_manager::instance()->add_metadata_field( $slug, $object_types, $args );
}
function x_add_metadata_multifield( $slug, $object_types = 'post', $args = array() ) {
    jb_metadata_manager::instance()->add_multifield( $slug, $object_types, $args );
}
function x_add_metadata_group( $slug, $object_types, $args = array() ) {
    jb_metadata_manager::instance()->add_metadata_group( $slug, $object_types, $args );
}